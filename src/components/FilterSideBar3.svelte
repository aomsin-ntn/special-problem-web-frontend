<script lang="ts">
    ...
    export interface Faculty extends Option {
        majors: Option[];
    }
    ...
    let { 
        ...
        faculties: Faculty[], 
        onFilterChange?: (filters: { years: string[], majors: string[] }) => void 
    } = $props();
    ...
    const recentYears = Array.from({ length: 5 }, (_, i) => String(currentBEYear - i));

    $effect(() => {
        if (onFilterChange) {
            onFilterChange({ years: selectedYears, majors: selectedMajors });
        }
    });
...
<aside class="w-full max-w-xs bg-white h-screen overflow-y-auto">
                        ...
                        {#each faculties as faculty}
                            <div class="faculty-group">
                                ...
                                {#if openFaculties[faculty.id]}
                                    <div class="options-container pl-3 mt-1 border-l-2 border-gray-200">
